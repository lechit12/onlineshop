...
function addToCart(name, price, quantity) {
  ...
  if (existingItem) {
    // Je�li produkt ju� istnieje w koszyku, zwi�ksz ilo��
...
// Wywo�aj funkcj� po za�adowaniu strony, aby zaktualizowa� zawarto�� koszyka
window.addEventListener("DOMContentLoaded", function() {
  updateCartTable();
  });

// Funkcja do z�o�enia zam�wienia
function placeOrder() {
  ...
  .then(data => {
    // Logika po z�o�eniu zam�wienia
  })
  .catch(error => {
    console.error('B��d podczas z�o�enia zam�wienia:', error);
  });
}
function removeFromCart(item) {
  var cartItems = JSON.parse(localStorage.getItem("cartItems")) || [];

  // Znajd� indeks produktu do usuni�cia
  var index = cartItems.findIndex(function(cartItem) {
    return cartItem.name === item.name;
  });

  if (index !== -1) {
    // Usu� produkt z koszyka
    cartItems.splice(index, 1);

    // Zapisz zmieniony koszyk w localStorage
    localStorage.setItem("cartItems", JSON.stringify(cartItems));

    // Zaktualizuj tabel� koszyka
    updateCartTable();
    ...
    function handleFormSubmission() {
      // Tutaj umieść logikę do obsługi formularza

      // Jeśli rejestracja przebiegnie pomyślnie, wywołaj funkcję popup()
      if (rejestracjaPomyslna()) {
        popup();
      }

      // Zwróć false, aby zapobiec domyślnej akcji formularza
      return false;
    }
    function popup() {
